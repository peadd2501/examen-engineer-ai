...
**Stack:** React + Vite + TypeScript · Fastify + TypeScript · PostgreSQL 16 · OpenRouter directo (sin framework de agentes) · Zod · decimal.js · SSE.

```
Frontend  (React + Vite, SSE por POST)
  ↓
Fastify API
  ↓
Agent Orchestrator
...
El orquestador corre en **dos fases**:

1. **AGENT** — bucle explícito de herramientas, `tool_choice: auto`, sin `response_format`. Recolecta evidencia. Termina cuando el modelo deja de pedir herramientas.
...
|---|---|
| `packages/contracts` | Esquemas Zod, helpers `decimal.js`, fórmulas financieras puras, matriz de riesgo, errores tipados. Única fuente de verdad numérica. |
...
| `apps/web` | Lista de solicitudes, análisis en vivo, dictamen, citas, autorización humana, métricas. |
| `database` | Migraciones SQL planas (10) y seed determinista. |
| `evaluation` | Los 10 fixtures, el harness, los smokes de proveedor. |
...
pnpm test                     # deterministas: fórmulas, riesgo, generador, veredicto de smoke
...
pnpm agent:smoke:finalizer    # ¿el modelo cierra por function call forzada?
...
| `OPENROUTER_MODEL` | Un modelo **concreto**. No `openrouter/free`: es un enrutador y cada corrida puede resolver a otro modelo, lo que arruina la reproducibilidad. El que realmente respondió queda en `agent_runs.resolved_model`. |
...
| `SEED` | Semilla del dataset y de la inferencia (`agent_runs.inference_seed`). |
| `CORS_ORIGIN` | Orígenes permitidos, separados por coma. |

---

## 4. Las cinco herramientas
...
| `buscar_politica` | Recuperación sobre el corpus (FTS + cobertura + relaciones) | Sí |
...
- **Al fallar:** se registra un `guardrail_finding` con el motivo (`POLICY_NOT_FOUND`, `TEXT_MISMATCH`, `SECTION_MISMATCH`), la decisión baja a comité y la cita falsa no llega a `decision_policy_citations`. Una cita válida pero nunca recuperada se marca (`CITATION_NOT_RETRIEVED`) sin bloquear.

### G2 — Indicadores autoritativos
...
- **Test real:** `G2: indicadores alterados por el modelo rechazan la persistencia` — adultera `razon_endeudamiento` y `cobertura_servicio_deuda`, verifica que ambos campos aparezcan en el error y que la tabla `decisions` quede sin filas.
- **Al fallar:** excepción antes de cualquier escritura. No hay dictamen a medias.

### G3 — Topes de monto

- **Riesgo:** recomendar más de lo que la política o la propia solicitud permiten.
- **Aplicación:** `max_allowed_amount` = mínimo de todos los topes aplicables, calculado desde la solicitud, nunca desde el modelo.
...
- **Al fallar:** rechazo antes de persistir; si alguien evita la aplicación, lo rechaza PostgreSQL.

### G4 — Autorización humana

- **Riesgo:** una recomendación con impacto queda firme sin firma.
- **Aplicación:** la regla se recalcula en backend (monto > Q250,000 · riesgo `ALTO`) e **ignora** lo que diga el modelo. El riesgo que consume es el calculado determinísticamente (ver §7).
...
- **Tests reales:** `G4: monto > Q250,000 nace PENDING_AUTHORIZATION aunque el modelo diga que no`, `G4: riesgo ALTO exige autorización sin importar el monto`, `G4: el nivel de riesgo del backend es el que decide, no el del modelo`, `G4: la base de datos impide que un dictamen con autorización nazca firme`.
- **Al fallar:** el dictamen nace en `PENDING_AUTHORIZATION` y solo `POST /api/decisions/:id/authorize` puede moverlo. El analista también puede rechazarlo manualmente.

### G5 — Entrada no confiable

- **Riesgo:** inyección de prompt en `destino_fondos`, el único campo de texto libre del solicitante.
- **Aplicación:** allowlist fija de herramientas (un nombre arbitrario o `registrar_dictamen` se rechazan sin ejecutar), detección de patrones registrada en `guardrail_findings`, y —desde FASE 3.3— el texto crudo **no entra al contexto decisional**: se sustituye por una representación normalizada (§7).
- **Tests reales:** `G5: los cinco fixtures adversariales quedan marcados`, `G5: tool abuse: un nombre de función arbitrario no se ejecuta`, `G5: registrar_dictamen no es alcanzable por el modelo`, `G5: los inputs decisionales son idénticos con y sin inyección`, `el destino de fondos crudo no aparece en ningún mensaje de ninguna fase`.
- **Al fallar:** la tool call se marca `ERROR` con `Herramienta no permitida`, el run continúa, y el intento queda en `guardrail_findings`. La inyección no cambia el resultado del dominio.
...
Los 15 tests de retrieval pasaban —y siguen pasando— en aislamiento. La evaluación end-to-end con LLM real mostró otra cosa: `buscar_politica` acertaba, pero **el modelo formulaba consultas deficientes** y decidía sobre la evidencia equivocada. El componente estaba bien; el uso del componente no.
...
Una decisión de crédito no puede depender de que el modelo acierte la consulta perfecta para enterarse de una regla básica.

### Escalabilidad

A ~500 políticas esto deja de caber y hay que volver a recuperación de verdad: híbrida (BM25/full-text como candidato base + embeddings para cobertura semántica, con `pgvector` o una base vectorial dedicada), filtros por metadata (categoría, vigencia), reranking cruzado sobre el top-N, y una **métrica de recall y precisión de citas** para comparar configuraciones en vez de opinar sobre ellas.

> **Nada de eso está implementado.** No hay embeddings, no hay `pgvector`, no hay reranking. Es el diseño que adoptaríamos, declarado como tal.

---

## 7. Structured output, riesgo y destino de fondos

### Structured output — evolución real
...
| Reparación dirigida por truncación | Un solo reintento con contexto compacto y `max_tokens` propio. CASE-09 seguía muriendo: `length` / 5000 tokens / ~5188 caracteres que no validaban; la reparación con 1400 tokens gastaba los 1400 en razonamiento y devolvía contenido vacío. |
...
Ahora el nivel lo calcula `calcularNivelRiesgo()` en `packages/contracts/src/risk.ts`, determinísticamente, desde la solicitud y los indicadores. Solo **dos situaciones** elevan a `ALTO`, y ambas tienen respaldo literal en el corpus:
...
- Se **persiste crudo** y es **visible** para el analista en la UI: es evidencia del expediente y borrarla sería peor.
- Se marca como no confiable y G5 lo inspecta (6 patrones de inyección).
- **No entra crudo al contexto decisional.** Se sustituye por `resumirDestinoFondos()`, que devuelve `{ categoria, longitud_caracteres, marcado_no_confiable }` con la categoría tomada de un vocabulario cerrado de 7 valores: `capital_trabajo`, `inventario`, `maquinaria_equipo`, `unidades_transporte`, `expansion_local`, `cuentas_por_cobrar`, `no_clasificado`.
...
- **En vivo:** timeline de actividad del agente, indicadores, dictamen, citas con su texto literal, panel G4 de autorización humana, panel de comité, métricas de ejecución (modelo resuelto, tokens, latencia, costo, `finish_reason`).
...
El mismo bug de cancelación (`request.raw.on('close')`, que dispara cuando termina de leerse el cuerpo, en vez de `reply.raw.on('close')`) existía en silencio en la ruta no-streaming desde FASE 3.
...
**Determinista — cerrado:**

```
pnpm typecheck   PASS   (strict + noUncheckedIndexedAccess)
pnpm test         70/70  contratos, indicadores, riesgo, generador, veredicto de smoke
pnpm test:web     23/23  componentes
pnpm test:db     118/118 loop (34), guardarraíles (39), retrieval (15), corpus (10),
                         payload del proveedor (9), SSE/CORS (6), idempotencia (5)
pnpm build:web   PASS
```

Todo corrido contra PostgreSQL 16 real con las 10 migraciones aplicadas y el seed sembrado.

**Evaluación con LLM real — PENDIENTE DE CIERRE:**

- CASE-01 — ejecución real **PASS** con Liquid.
- CASE-04 — ejecución real **PASS** con Liquid.
- CASE-09 — **pendiente de nueva prueba** con el finalizer forzado; bloqueado por límite de cuota de OpenRouter.
- Los 10 casos completos — **pendientes de cuota**.

No hay un resultado 10/10 y este README no lo afirma. Los números deterministas de arriba son reales y reproducibles; el desempeño del modelo sobre los 10 fixtures no está cerrado.

**Suite de humo antes de gastar los diez:**

```bash
pnpm agent:smoke              # ¿la key, el modelo y el structured output funcionan?
pnpm agent:smoke:finalizer    # ¿el modelo cierra por function call forzada? (payload real de CASE-09)
pnpm eval CASE-01
```

`pnpm agent:smoke:finalizer` reproduce en una sola llamada el payload real de finalización de un caso (por defecto EVAL-CASE-09) y usa **el mismo parser que producción**. Un HTTP 200 no es PASS: exige `finish_reason` compatible, exactamente una llamada, nombre correcto, argumentos presentes, JSON válido y esquema cumplido. Salida 0 = PASS, 1 = el modelo no sabe finalizar así, 2 = configuración o datos.

---

## 11. Limitaciones actuales

1. **La evaluación con LLM real no está cerrada** (ver §10). Es la limitación principal.
2. **No se emite riesgo `BAJO`** — el corpus no define condiciones suficientes y no se inventó el umbral (§7).
3. **POL-5.3 no se puede evaluar con precisión** — requiere un destino de fondos más detallado del que el vocabulario cerrado permite; la solución correcta es un campo estructurado en el formulario, no un mejor clasificador de texto libre (§7).
4. **Sin embeddings ni recuperación híbrida** — deliberado para 30 políticas, insuficiente a ~500 (§6).
5. **Modelos gratuitos al límite** — el finalizer forzado existe porque `response_format` no era estable con ellos. Un modelo con soporte sólido de function calling haría este flujo mucho más holgado.
6. **Sin autenticación ni roles** — fuera de alcance. La autorización humana es un endpoint separado, pero no identifica a la persona.
7. **Sin CI/CD ni despliegue** — fuera de alcance.
